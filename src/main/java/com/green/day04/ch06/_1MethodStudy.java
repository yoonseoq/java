...
public class _1MethodStudy {
    ...
    만든 메소드를 사용하는 행위를 '메소드를 호출했다'고 표현
    public static은 나중에
    ...
    public static void main(String[] args)(리턴타입+매소드명+파라미터):선언부
    ...
    public static void main(String[] args) {
        sum(8, 7);   //sum메소드 호출 >> 8이 n1,7이 n2로 대입 void
        sum(18, 7);  //sum메소드 호출
    ...
    //메소드를 만들었으니까 sum이라는 이름의 static 메소드를 정의
    static void sum(int n1, int n2) //선언부
    {
        System.out.printf("%d + %d = %d\n", n1, n2, (n1 + n2));
    }

    static int returnSum(int n1, int n2) { //똑같은 개수의 메소드는
        return n1 + n2; //호출한 자리가 12이됨 ,변수선언할때 쓰이는 거는 return
        //메소드는 함수같은 건가요
    }
